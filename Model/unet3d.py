...
class up_conv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, k_size=3, stride=1, padding=1, bias=True):
        ...
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2).to(device_2),
            nn.Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=k_size,
                      stride=stride, padding=padding, bias=bias).to(device_2),
            nn.BatchNorm3d(num_features=out_channels).to(device_2),
            nn.ReLU(inplace=True))
...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch=1, out_ch=1):
        ...
        self.Up5 = up_conv(filters[4], filters[3]).to(device_2)
        self.Up_conv5 = conv_block(filters[4], filters[3]).to(device_2)
